...
using DPWrestlingScoreboard.Windows;
...
namespace DPWrestlingScoreboard
{
    public partial class MainWindow : Window
    {
        ...
        // Время (в секундах)
        private int _mainTimeSeconds = 360; // Период 1 по умолчанию
        private int _redTimeSeconds = 30;   // 0:30
        private int _blueTimeSeconds = 30;  // 0:30

        private int _period1DurationSec = 360;
        private int _period2DurationSec = 180;
        ...
        private void SetupUserPermissions()
        {
            ...
            {
                weightCategoriesButton.Visibility = Visibility.Visible;
            ...
            if (CurrentUser.IsGuest)
            {
                ...
                toggleScoreboardButton.Visibility = Visibility.Collapsed;
        ...
        private void LoadDataFromDb()
        {
            try
            {
                ...
                _weightCategories = context.WeightCategories
                    .AsNoTracking()
        ...
        private void StopTimersOnSignal()
        {
            _mainTimer.Stop();
            _isMainTimerRunning = false;
            _redTimer.Stop();
        ...
        private void Period1_Click(object sender, RoutedEventArgs e)
        {
            ...
            // Длительность периода из выбранного режима
            _mainTimeSeconds = _period1DurationSec;
            UpdateMainTimerDisplay();
            UpdateScoreboardTimer();
        ...
        private void Period2_Click(object sender, RoutedEventArgs e)
        {
            ...
            // Длительность периода из выбранного режима
            _mainTimeSeconds = _period2DurationSec;
            UpdateMainTimerDisplay();
            UpdateScoreboardTimer();
            UpdateScoreboardPeriod();
        }
        ...
        #region Match duration (6+3 vs 4+4)
        private void MatchDurationStandard_Checked(object sender, RoutedEventArgs e)
        {
            if (!IsLoaded) return;
            _period1DurationSec = 360;
            _period2DurationSec = 180;
            ApplyCurrentPeriodDurationFromSettings();
        }

        private void MatchDurationFourFour_Checked(object sender, RoutedEventArgs e)
        {
            if (!IsLoaded) return;
            _period1DurationSec = 240;
            _period2DurationSec = 120;
            ApplyCurrentPeriodDurationFromSettings();
        }

        private void ApplyCurrentPeriodDurationFromSettings()
        {
            _mainTimeSeconds = _currentPeriod == 1 ? _period1DurationSec : _period2DurationSec;
            UpdateMainTimerDisplay();
            UpdateScoreboardTimer();
        }
        #endregion
        ...
        {
            RecordMatchWinner(isRedWinner: true);
        ...
        {
            RecordMatchWinner(isRedWinner: false);
        ...
        private void RecordMatchWinner(bool isRedWinner)
        {
            if (weightCategoryComboBox.SelectedItem is not ComboBoxItem categoryItem)
                return;

            int categoryId = (int)categoryItem.Tag!;
            int? winnerId = GetSelectedWrestlerId(isRedWinner ? redWrestlerComboBox : blueWrestlerComboBox);
            int? loserId = GetSelectedWrestlerId(isRedWinner ? blueWrestlerComboBox : redWrestlerComboBox);

            if (winnerId == null || loserId == null || winnerId == loserId)
                return;

            int winnerScore = isRedWinner ? _redScore : _blueScore;
            int loserScore = isRedWinner ? _blueScore : _redScore;
            int victoryPoints = VictoryPointsCalculator.Calculate(winnerScore, loserScore);

            TournamentStateService.Current.RecordMatchResult(
                categoryId,
                winnerId.Value,
                loserId.Value,
                victoryPoints);
        ...
        private static int? GetSelectedWrestlerId(System.Windows.Controls.ComboBox comboBox)
        {
        ...
        private void Tournament_Click(object sender, RoutedEventArgs e)
        {
            ...
            {
                System.Windows.MessageBox.Show(
                    "Турнирные таблицы недоступны в режиме гостя.",
                    "Доступ запрещён",
                    MessageBoxButton.OK,
        ...
        private void WeightCategories_Click(object sender, RoutedEventArgs e)
        {
            if (!CurrentUser.IsAdmin) return;

            var window = new WeightCategoryManagementWindow { Owner = this };
            window.ShowDialog();
        ...
        {
            string message = "ИНФОРМАЦИОННАЯ СИСТЕМА ДЛЯ СУДЕЙСТВА СОРЕВНОВАНИЙ ПО ГРЕКО-РИМСКОЙ И ВОЛЬНОЙ БОРЬБЕ\n" +
                           "Версия: 1.0.0\n\n" +
                           "Махин Николай, ИСПП-21, 2026";
